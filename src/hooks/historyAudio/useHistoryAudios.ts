...
export function useHistoryAudios(pointId: string) {
  ...
  const fetchAudios = useCallback(async () => {
    ...
    try {
      const res = await adminHistoryAudioService.getAllByPoint(pointId);
      setAudios(res.data);
    } catch (err) {
  ...
  const createAudio = async (data: CreateHistoryAudioRequest) => {
    const res = await adminHistoryAudioService.create(pointId, data);
    await fetchAudios();
    return res.data;
  };

  const updateAudio = async (id: string, data: CreateHistoryAudioRequest) => {
    const res = await adminHistoryAudioService.update(pointId, id, data);
    await fetchAudios();
    return res.data;
  };

  const deleteAudio = async (id: string) => {
    await adminHistoryAudioService.delete(pointId, id);
    await fetchAudios();
  };
