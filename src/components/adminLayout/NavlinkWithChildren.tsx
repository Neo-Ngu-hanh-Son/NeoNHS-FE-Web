  ...
  return (
    <div key={navItem.path} className="relative group">
      ...
      >
        ...
        {!isCollapsed && <span className="material-symbols-outlined shrink-0 text-sm">arrow_drop_down</span>}
      </div>
        ...
        <div className="flex flex-col gap-2 w-[220px] bg-sidebar-bg border border-white/10 rounded-lg p-2 shadow-lg">
          {navItem.children.map((child) => (
            <NavLink
              ...
              to={child.path}
              className={({ isActive }) =>
                `flex items-center gap-3 p-2.5 rounded-lg transition-all cursor-pointer ${isActive
