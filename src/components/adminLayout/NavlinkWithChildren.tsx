...
export default function NavlinkWithChildren({ navItem, isCollapsed }: Props) {
  ...
  return (
    <div key={navItem.path} className="relative group">
      <div
        className={`flex items-center gap-3 p-2.5 rounded-lg text-white/70 hover:bg-white/5 hover:text-white cursor-pointer transition-all 
          ...
          ${isParentActive ? 'bg-white/10 text-white' : 'text-white/60 hover:bg-white/5 hover:text-white'}
        `}
        title={isCollapsed ? navItem.label : ''}
      >
        <span className="shrink-0">{navItem.icon}</span>
        {!isCollapsed && <p className="text-sm font-medium flex-1">{navItem.label}</p>}
    ...
    "
      >
        <div className="flex flex-col gap-1 w-[220px] bg-sidebar-bg border border-white/10 rounded-xl p-2 shadow-lg">
          {navItem.children?.map((child) => (
            <NavLink
              key={child.path}
              to={child.path}
              className={({ isActive }) =>
                `flex items-center gap-3 p-2.5 rounded-lg transition-colors cursor-pointer ${isActive ? 'bg-white/10 text-white' : 'text-white/60 hover:bg-white/5 hover:text-white'
                }`
              }
            >
