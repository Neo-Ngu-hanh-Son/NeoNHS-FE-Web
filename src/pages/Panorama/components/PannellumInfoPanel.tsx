...
interface PannellumInfoPanelProps {
  isOpen: boolean;
  onClose: () => void;
}

/**
 * Info panel for the Pannellum panorama screen.
 * Same content & drag-to-close behavior as the PSV InfoPanel,
 * using pannellum-specific CSS class names.
 */
export default function PannellumInfoPanel({ isOpen, onClose }: PannellumInfoPanelProps) {
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent) => {
    ...
    const clamped = Math.max(0, e.clientY - startY.current);
  ...
  const handlePointerUp = useCallback(() => {
    ...
    if (translateYRef.current > 80) onClose();
    translateYRef.current = 0;
    setTranslateY(0);
  }, [onClose]);
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className={`pannellum-info-backdrop ${isOpen ? "pannellum-info-backdrop--visible" : ""}`}
        onClick={onClose}
      />

      {/* Panel */}
      <div
        className={`pannellum-info-panel ${isOpen ? "pannellum-info-panel--open" : ""}`}
        style={{
          transform:
      ...
      >
        {/* Drag handle */}
        <div
          className="pannellum-info-panel__handle"
          onPointerDown={handlePointerDown}
          onPointerMove={handlePointerMove}
          onPointerUp={handlePointerUp}
          onPointerCancel={handlePointerUp}
        >
          <div className="pannellum-info-panel__handle-bar" />
        </div>

        {/* Body */}
        <div className="pannellum-info-panel__body">
          {/* Location header */}
