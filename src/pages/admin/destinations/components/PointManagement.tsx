                ...
                <div className="rounded-xl border bg-white flex-1 overflow-hidden flex flex-col shadow-sm">
                        ...
                        <Table className="text-sm">
                            ...
                            <TableBody>
                                {loading ? (
                                        ...
                                        <TableCell colSpan={6} className="h-60 text-center">
                                            <div className="flex flex-col items-center justify-center space-y-3 py-10 opacity-60">
                                                ...
                                                <div className="space-y-1">
                                                    <p className="text-base font-semibold text-gray-600">No points discovered yet</p>
                                                    <p className="text-sm text-gray-400">Add your first point of interest to start managing.</p>
                                                </div>
                                            </div>
                                        </TableCell>
                                    </TableRow>
                                    ...
                                    allPoints.map((p, idx) => (
                                        <TableRow key={p.id} className="hover:bg-primary/5 transition-colors duration-200 group">
                                            ...
                                            <TableCell>
                                                <Badge variant="secondary" className="bg-gray-100 text-gray-600 border-none font-medium px-2 py-0">
                                                    {p.type}
                                                </Badge>
                                            </TableCell>
                                            <TableCell className="text-center">
                                                <span className="font-mono text-gray-500">{p.estTimeSpent ? `${p.estTimeSpent}m` : '-'}</span>
                                            </TableCell>
                                            <TableCell className="text-right pr-6">
                                                <div className="flex items-center justify-end gap-1">
                                                    <TooltipProvider>
                                                        <Tooltip>
                                                            <TooltipTrigger asChild>
                                                                <Button variant="ghost" size="icon" className="h-9 w-9 hover:bg-emerald-50 hover:text-emerald-600 rounded-full" onClick={() => onFocus(p.latitude, p.longitude)}>
                                                    ...
                                                    <TooltipProvider>
                                                        <Tooltip>
                                                            <TooltipTrigger asChild>
                                                                <Button variant="ghost" size="icon" className="h-9 w-9 hover:bg-amber-50 hover:text-amber-600 rounded-full" onClick={() => onEditPoint(p)}>
                                                    ...
                                                    <TooltipProvider>
                                                        <Tooltip>
                                                            <TooltipTrigger asChild>
                                                                <Button variant="ghost" size="icon" className="h-9 w-9 hover:bg-destructive/10 hover:text-destructive rounded-full" onClick={() => onDeletePoint(p.id)}>
                    ...
                    <div className="bg-gray-50/50 border-t px-6 py-3 flex items-center justify-between">
                        <div className="text-xs text-slate-500 font-medium">
                            Showing <span className="font-bold text-slate-700">{allPoints.length}</span> of <span className="font-bold text-slate-700">{pagination.totalElements}</span> results
                        </div>
