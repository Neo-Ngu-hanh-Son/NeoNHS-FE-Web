...
export function AdminDashboardPage() {
  ...
  const [registrationsData, setRegistrationsData] = useState<RegistrationTrendResponse | null>(
    null,
  );
  ...
  const loadAllData = async () => {
    setLoading(true);
    try {
      const [kpiData, revData, actData, salesData, topEvents, topWorkshops, regData, recentData] =
        await Promise.all([
        ...
        <Col span={24} style={{ display: "flex", flexDirection: "column" }}>
          ...
          <Row gutter={[16, 16]} style={{ marginTop: 16, flex: 1 }}>
            <Col xs={24} lg={9} style={{ display: "flex" }}>
              <div style={{ flex: 1, width: "100%" }}>
                <SalesByTypeChart salesByType={salesByType} activityStatus={activityStatus} />
              </div>
            </Col>
